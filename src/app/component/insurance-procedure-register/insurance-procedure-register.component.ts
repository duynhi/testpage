...
@Component({
  selector: 'app-insurance-procedure-register',
  templateUrl: './insurance-procedure-register.component.html',
  styleUrls: ['./insurance-procedure-register.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class InsuranceProcedureRegisterComponent implements OnInit {
  collapsedAddress = false;
  collapsedPhone = false;
  collapsedName = false;
  collapsedPayType = false;
  collapsedPayTime = false;
  collapsedOther = false;
  collapsedNameDest = false;
  indexFamily: number;
  myPageUserInfo: any;
  familyList = [];
  familyListInput = [];
  ...
  changeStatusPhone() {
    ...
    if (this.collapsedPhone) {
      this.requireDate();
    } else {
      if (this.collapsedAddress) {
        this.requireDate();
      } else {
        this.clearValidateDate();
      }
    }
  }
  ...
  changeStatusPayType() {
    this.collapsedPayType = !this.collapsedPayType;
    // this.clearValidateDate();
  }

  changeStatusNameDest() {
    this.collapsedNameDest = !this.collapsedNameDest;
    // this.clearValidateDate();
  }

  changeStatusOther() {
    this.collapsedOther = !this.collapsedOther;
    // this.clearValidateDate();
  }

  checkChoooseStatus() {
    if (this.collapsedPhone || this.collapsedAddress || this.collapsedName
      || this.collapsedPayType || this.collapsedNameDest || this.collapsedOther) {
      return true;
    } else {
      return false;
    }
  ...
  requireDate() {
    this.insuranceGroup.controls.year.setValidators([Validators.required]);
    this.insuranceGroup.controls.month.setValidators([Validators.required]);
    this.insuranceGroup.controls.day.setValidators([Validators.required]);
  }

  clearValidateDate() {
    this.insuranceGroup.controls.year.clearValidators();
    this.insuranceGroup.controls.month.clearValidators();
    this.insuranceGroup.controls.day.clearValidators();
    this.insuranceGroup.controls.year.setErrors(null);
    this.insuranceGroup.controls.month.setErrors(null);
    this.insuranceGroup.controls.day.setErrors(null);
    this.insuranceGroup.controls.year.setValue('');
    this.insuranceGroup.controls.month.setValue('');
    this.insuranceGroup.controls.day.setValue('');
    this.insuranceGroup.controls.year.reset('');
    this.insuranceGroup.controls.month.reset('');
    this.insuranceGroup.controls.day.reset('');
    this.insuranceGroup.controls.year.markAsPristine();
    this.insuranceGroup.controls.month.markAsPristine();
    this.insuranceGroup.controls.day.markAsPristine();
    console.log('done', this.insuranceGroup.controls.year);
  ...
  changeDesPerson(familyNo) {
    ...
    if (familyNo !== -1) {
      ...
      this.insuranceGroup.controls.familyInput.setValue('');
  ...
  chooseShowDestination(value) {
    ...
    if (!this.chooseDestination) {
      ...
      this.insuranceGroup.controls.destZip1.setValue('');
      this.insuranceGroup.controls.destZip2.setValue('');
      this.insuranceGroup.controls.destAddressTown.setValue('');
      this.insuranceGroup.controls.destAddressBldg.setValue('');

      console.log('aaaa', this.insuranceGroup.controls.destZip1);
    } else {
  ...
  getListKeiyaku(familyNo) {
    ...
    this.baseService.post(this.API_URLS.getListKeiyaku, value).subscribe(res => {
      if (res.errorList === null || (res.errorList && res.errorList.length === 0)) {
        ...
        console.log('listStatusKeiyakuFamily', this.listStatusKeiyakuFamily);
  ...
  changeColourFamilyInput(i, familyNo, familyInputID) {
    ...
    this.statusColorFamilyInput.forEach((family) => {
      family.status = false;
    });
  ...
  initForm() {
    this.insuranceGroup = this.formBuilder.group({
      ...
      newZip1: ['', [Validators.pattern('^[0-9]+$')]],
      newZip2: ['', [Validators.pattern('^[0-9]+$')]],
      newAddressPref: [''],
      newAddressTown: ['', Validators.maxLength(400)],
      newAddressBldg: ['', Validators.maxLength(200)],
      destZip1: ['', [Validators.pattern('^[0-9]+$')]],
      destZip2: ['', [Validators.pattern('^[0-9]+$')]],
      destAddressPref: [''],
      destAddressTown: [''],
      destAddressBldg: [''],
      ...
      familyInput: ['', [Validators.maxLength(50)]],
  ...
  saveInsurance() {
    ...
    const value = {
      ...
      insuranceProcedureInfo: {
        ...
        destF: this.chooseDestination ? 0 : 1,
        ...
        phoneF: this.chooseDestinationPhone ? 0 : 1,
    ...
    if (this.collapsedName) {
      request = request + 4;
    }
    if (this.collapsedNameDest) {
      request = request + 8;
    }
    if (this.collapsedPayType) {
      request = request + 16;
    }
    value.insuranceProcedureInfo.requestF = request.toString();

    // if (this.collapsedPhone && !this.collapsedAddress) {
    //   value.insuranceProcedureInfo.requestF = '2';
    // } else if (this.collapsedAddress && !this.collapsedPhone) {
    //   value.insuranceProcedureInfo.requestF = '1';
    // } else if (this.collapsedAddress && this.collapsedPhone) {
    //   value.insuranceProcedureInfo.requestF = '3';
    // }
    if (this.collapsedPhone || this.collapsedAddress) {
      value.insuranceProcedureInfo.destF = null;
    }

    this.baseService.post(this.API_URLS.saveInsurance, value).subscribe(res => {
      // this.loading = false;
      if (res.errorList === null || (res.errorList && res.errorList.length === 0)) {
        this.router.navigate(['/insurance-procedure-regist-success/1']);
      }
      if (res.errorList && res.errorList.length > 0) {
        res.errorList.forEach(error => {
          this.showError(error.errMessage);
  ...
  yuubinbango(){
    let z1 = this.insuranceGroup.controls.newZip1.value;
    let z2 = this.insuranceGroup.controls.newZip2.value;
    const value = {
        zip1: z1,
        zip2: z2
    }
      ...
      this.baseService.post(this.API_URLS.address, value).subscribe(res => {
        this.loading = false;
        if (res.errorList === null || (res.errorList && res.errorList.length === 0)) {
          this.insuranceGroup.controls.newAddressPref.setValue(res.addressInfo.addressPref);
          this.insuranceGroup.controls.newAddressTown.setValue(res.addressInfo.addressTown);
